...
const LaUAM = () => {
    ...
    const { footerLaUamData } = useGlobalState();
    ...
    return (
        <>
            {/* Vista de escritorio */}
            <div className="hidden w-full lg:max-w-[270px] lg:flex flex-col justify-center items-start">
                <h1 className="text-[16px] leading-[28px] font-bold mb-4">
                    {footerSectiosData?.data?.[1]?.sectionTitle || 'Título no disponible'}
                </h1>
                <ul className="w-full flex flex-col justify-start items-start text-[16px] leading-[28px] text-primary gap-4">
                    {footerLaUamData && footerLaUamData.length > 0 ? (
                        footerLaUamData
                            .sort((a, b) => a.id - b.id)
                            .map((link, index) => (
            ...
            <div className="lg:hidden w-full border-b border-gray-200">
                ...
                >
                    <h1 className="font-bold">La UAM</h1>
                    <IoTriangle
                        size={14}
